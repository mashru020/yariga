...
const PropertyDetails = () => {
  ...
  const handleDeleteProperty = () => {
    const response = window.confirm(
      "Are you sure you want to delete this propoerty?",
    );
    if(response) {
      mutate(
        {
    ...
    <Box borderRadius="15px" padding="20px" bgcolor="#fcfcfc" width="fit-content">
      ...
      <Box mt="20px" display="flex" flexDirection={{xs:'column', lg: 'row'}} gap={4}>
        ...
        <Box width="100%" flex={1} maxWidth={326} display="flex" flexDirection="column" gap="20px">
          <Stack mt={2} p={2} direction="column" justifyContent="center" alignItems="center" border="1px solid #e4e4e4" borderRadius={2}>
            <Stack mt={2} justifyContent="center" alignItems="center" textAlign="center">
              <img 
                src={checkImage(propertyDetails.creator.avatar)
              ...
              <Stack mt="15px" direction="row" alignItems="center" gap={1}>
                <Place sx={{color:'#808191'}}/>
                <Typography>{propertyDetails.creator.address ? propertyDetails.creator.address : "Dhaka" }</Typography>
              </Stack>
